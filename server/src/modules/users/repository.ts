...
export const getUserByRole = (role: Role): Promise<IUser> => {
...
export const updateMultipleUsers = (filters = {}, data: Partial<IUser>): Promise<FindAndUpdateResult> => {
  data.email &&= data.email.toLowerCase();
  return User.updateMany(filters, data, { new: true }).select(scrubbable).lean();
};
